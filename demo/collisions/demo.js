...
for (let i = 0; i < numCircles; i++) {
    let c = sci.circle({
        x: Math.random() * (system.width - 20) + 10,
        y: Math.random() * (system.height - 20) + 10,
        mass: 10,
        // force: {
    ...
    system.add(sci.boundaryConstraint(c, system));
    circles.push(c);
}
...
function loop() {
    circles.forEach(c => {
        if (c.position.x + c.radius >= system.width) {
        }
        ...
        if (c.position.y + c.radius >= system.height) {
...
document.addEventListener('mouseup', function(e) {
    if (squareDown) {
        ...
        if (e.clientY < window.innerHeight - 100) {
            ...
            system.add(rect);
            system.add(sci.boundaryConstraint(rect, system));
        }
        squareDown = false;
    }
    if (circleDown) {
        ...
        if (e.clientY < window.innerHeight - 100) {
            system.add(circ);
            system.add(sci.boundaryConstraint(circ, system));
        }
        circleDown = false;
    }
    if (waveDown) {
    ...
    if (polyDown) {
        ...
        if (e.clientY < window.innerHeight - 100) {
            ...
            system.add(p);
            system.add(sci.boundaryConstraint(p, system));
        }
    }
});
